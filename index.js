...
console.log(API_ID && API_HASH ? "API ID/HASH loaded" : "API ID/HASH not added");
...
async function getHtml(url) {
  const res = await axios.get(url, {
    timeout: 20000,
    headers: { "User-Agent": "Mozilla/5.0 LegalMovieBot/1.0" }
  });
  return res.data;
}
...
bot.onText(/\/addsite (.+)/, (msg, match) => {
  ...
  if (parts.length < 2) return bot.sendMessage(msg.chat.id, "Usage: /addsite SiteName | HomeURL");

  const [name, homeUrl] = parts;
  const sites = loadSites();
  sites.push({ name, homeUrl });
  saveSites(sites);

  bot.sendMessage(msg.chat.id, `Site added: ${name}`);
});
...
bot.onText(/\/sites/, (msg) => {
  ...
  const sites = loadSites();
  if (!sites.length) return bot.sendMessage(msg.chat.id, "No sites added.");
...
bot.onText(/\/delsite (.+)/, (msg, match) => {
  ...
  const newSites = oldSites.filter(s => s.name.toLowerCase() !== name.toLowerCase());

  saveSites(newSites);

  bot.sendMessage(msg.chat.id, oldSites.length === newSites.length ? "Site not found." : "Site deleted.");
});

async function searchSite(site, query) {
  const html = await getHtml(site.homeUrl);
  ...
  $("a").each((_, el) => {
    ...
    if (cleanName(title).includes(q)) {
      const url = absoluteUrl(site.homeUrl, href);
      if (url) results.push({ site: site.name, title, url });
    }
  });
...
bot.on("message", async (msg) => {
  ...
  const sites = loadSites();
  if (!sites.length) return bot.sendMessage(chatId, "No sources added yet.");
  ...
  const keyboard = allResults.map((r, i) => [
    { text: `${r.title} - ${r.site}`, callback_data: `result_${i}` }
  ]);
...
bot.on("callback_query", async (query) => {
  ...
  if (data.startsWith("result_")) {
    ...
    if (!selected) return bot.answerCallbackQuery(query.id, { text: "Expired." });
    ...
    const keyboard = links.map((l, i) => [
      { text: l.quality, callback_data: `quality_${i}` }
    ]);
